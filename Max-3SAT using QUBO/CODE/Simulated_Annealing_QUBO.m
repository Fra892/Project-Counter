...
for c = 1:size(clauses,1)     % we loop over the number of rows
    ...
    switch num_neg
        case 0 % Type 1: (xi OR xj OR xk)
            % QUBO approximation from the paper (Table V, case a)
            local_Q = [-1,  1,  1;
                        0, -1,  1;    
                        0,  0, -1];
            % oreder doesn't matter %
        case 1 % Type 2: (xi OR xj OR NOT xk)
            % QUBO approximation from the paper (Table V, case b)
            local_Q = [0,  1,  -1;
                       0,  0,  -1;   
                       0,  0,   1];
        ...
        case 2 % Type 3: (xi OR NOT xj OR NOT xk)
            % QUBO approximation from the paper (Table V, case c)
            local_Q = [1, -1, -1;
                       0,  0,  1;    
                       0,  0,  0];
            % we exchange literals wlog 
        ...
        case 3 % Type 4: (NOT xi OR NOT xj OR NOT xk)
            % QUBO approximation from the paper (Table V, case d)
            local_Q = [-1,  1,  1;
                        0, -1,  1;    
                        0,  0, -1];
...
% final_temp = initial_temp * (final_temp/initial_temp)^(1 /
% max_iter)^(max_iter) = final_temp;
...
% print the results %
fprintf('Number of satisfied clauses: %d out of %d\n', satisfied_clauses, size(clauses,1));
end
